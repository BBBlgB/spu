...
ArrayRef MatMulAA_simple::proc(KernelEvalContext* ctx, const ArrayRef& x,
  ...
  if(rank == 2){
    ...
    auto c1 = ring_sub(ring_mmul(ring_add(a0, a1), ring_add(b0, b1), m, n, k), c0);
    ...
    if(rank <= 1){
      ...
      pforeach(0, size, [&](int64_t idx){
        ...
        for(int i = (int)(k - 1); i >= 0; i--){
          if(_eta__[idx] == 0){
            w = (p + _dp_x[idx * k + i] + rank * r_bits[i] - 2 * r_bits[i] * _dp_x[idx * k + i]) % p;
            _c[idx * k + i] = (p + rank * r_bits[i] - _dp_x[idx * k + i] + rank + w_total) % p;
            w_total = (w_total + w) % p;
          } else if(_eta__[idx] == 1 && _r[idx] != L_1){
            w = (p + _dp_x[idx * k + i] + rank * t_bits[i] - 2 * t_bits[i] * _dp_x[idx * k + i]) % p;
            _c[idx * k + i] = (p - rank * t_bits[i] + _dp_x[idx * k + i] + rank + w_total) % p;
            w_total = (w_total + w) % p;
          } else{
            ...
            if(i != 1){
              _u[idx] = _u[idx] % p;
              _c[idx * k + i] = (1 - rank) * (_u[idx * k + i] + 1) - rank * _u[idx * k + i];
            }
            else {
              _u[idx] = _u[idx] % p;
    ...
    if(rank == 2){
      ...
      pforeach(0, size, [&](int64_t idx){
        ...
        for(size_t bit = 0; bit < k; bit++){
          ...
          _dp_x_p1[idx * k + bit] = (U)(dp_x[bit] + p - _dp_x_p0[idx * k + bit]);
        }

        //split delta in Z_(L_1)
        _delta_p0[idx] = dis(gen);
        _delta_p1[idx] = _delta[idx] - _delta_p0[idx];
        if(_delta[idx] < _delta_p0[idx]) _delta_p1[idx] -= (U)1; //when overflow
      });//end foreach
...
ArrayRef Msb::proc(KernelEvalContext* ctx, const ArrayRef& in) const{
  ...
  DISPATCH_ALL_FIELDS(field, "securenn.msb", [&](){
    ...
    if(rank <= 1){
      ...
      gamma = ring_sub(ring_sub(beta_, ring_mul(beta, beta_)), ring_mul(beta, beta_));
      delta = ring_sub(ring_sub(lsb_x, ring_mul(lsb_x, lsb_r)), ring_mul(lsb_x, lsb_r));
      if(rank == 1){
      ...
      // Zi = Ci + (X - A) * Bi + (Y - B) * Ai + <(X - A) * (Y - B)>
      auto theta = ring_add(ring_add(ring_mul(gamma_a, beaver_b), ring_mul(delta_b, beaver_a)), beaver_c);
      if (rank == 0)
        // z += (X-A) * (Y-B);
